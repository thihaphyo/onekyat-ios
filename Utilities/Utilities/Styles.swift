...
public func applySketchShadow(
    ...
    blur: CGFloat,
    spread: CGFloat
    ,viewCornerRadius: CGFloat = 0) -> (UIView) -> Void {
    return {
        ...
        if viewCornerRadius != 0 {
            $0.layer.cornerRadius = viewCornerRadius
        }
    }
}
...
public func colorCodedStyle(baseColor: UIColor, codedColor: UIColor, _ query: String, baseFont: UIFont, codedFont: UIFont) -> (UIButton) -> Void {
    return {
...
public func colorCodedStyle(baseColor: UIColor, codedColor: UIColor, _ query: [String], baseFont: UIFont, codedFont: UIFont) -> (UILabel) -> Void {
    return { textLabel in
        ...
        let attributedText = NSMutableAttributedString(string: text,
                                                       attributes: [NSAttributedString.Key.foregroundColor: baseColor, .font:baseFont])
        query.forEach { txt in
            let queryRange = (attributedText.string as NSString).range(of: txt)
            attributedText.addAttributes([NSAttributedString.Key.foregroundColor: codedColor, .font: codedFont], range: queryRange)
...
public func underLineStyle(baseColor: UIColor, codedColor: UIColor, _ query: [String], baseFont: UIFont, codedFont: UIFont) -> (UILabel) -> Void {
    return { textLabel in
        let text = textLabel.text ?? ""
        ...
        let attributedText = NSMutableAttributedString(string: text,
                                                       attributes: [NSAttributedString.Key.foregroundColor: baseColor, .font:baseFont, NSAttributedString.Key.paragraphStyle: paragraphStyle])
        query.forEach { txt in
            let queryRange = (attributedText.string as NSString).range(of: txt)
            attributedText.addAttributes([NSAttributedString.Key.foregroundColor: codedColor, .font: codedFont], range: queryRange)
...
public func shadowImageStyle(color: UIColor?, offset: CGSize = .init(width: 1, height: 1), radius: CGFloat = 10) -> (UIView) -> Void {
    return {
...
public func wrapStyle(padding: UIEdgeInsets, background: UIColor = .clear) -> (UIView) -> UIView {
    return {
        let wrapper = UIView()
